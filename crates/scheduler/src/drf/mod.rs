...
impl Scheduler for DrfScheduler {
    fn schedule(
        ...
        let mut dominant_shares: PriorityQueue<TaskId, Reverse<OrderedFloat<f64>>> =
            PriorityQueue::new();
        ...
        for task in tasks {
            let max_dom_resource = 0.0_f64
                ...
                .max(if total_gpus != 0 {
                    task.resources.gpu as u32 as f64 / total_gpus as f64
                } else {
                    0.0
                });
        ...
        for (task_id, _) in dominant_shares.clone().into_sorted_iter() {
            match self.get_best_fit(
                ...
                Ok(assignment) => {
                    // Subtract resources
                    if let Some(node) = transformed_nodes
                        .iter_mut()
    ...
    fn get_best_fit(
        ...
        for node in nodes {
            let min_resource_remaining = 100.0_f64
                ...
                .min(
                    (node.available_memory_bytes as f64 - task.resources.memory_bytes as f64)
                        / node.available_memory_bytes as f64,
                )
                .min(
                    (node.available_disk_bytes as f64 - task.resources.disk_bytes as f64)
                        / node.available_disk_bytes as f64,
                )
                .min(if !task.resources.gpu {
                    f64::INFINITY
                } else {
        ...
        match node_id {
            Some(id) => Ok(Assignment {
                task_id: task.id.clone(),
                node_id: id,
            }),
            _ => Err(SchedulerError::InsufficientResources(format!(
                "Insufficient resources for task {}",
                task.id
            ))),
        }
    }
